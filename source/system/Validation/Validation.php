...
class Validation implements ValidationInterface
{
    ...
    /**
     * Sets the error for a specific field or for multiple fields. Used by custom validation methods.
     *
     * @param array $errors
     *
     * @return ValidationInterface
     *
     * @throws InvalidArgumentException If error field or error message not of type string.
     */
    public function setErrors(array $errors): ValidationInterface
    {
        ...
        if (!$isAssoc($errors)) {
            throw ValidationException::forGroupNotAssocArray();
        }

        foreach ($errors as $field => $mssg) {
            if (!is_string($field) || !is_string($mssg)) {
                throw new InvalidArgumentException('Validation error field and message must be of type string.');
            }
            $this->errors[$key] = $mssg;
        }
